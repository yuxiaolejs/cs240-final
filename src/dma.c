...
static dma_cb_t cb[79] __attribute__((aligned(32)));
...
void run_dma()
{
    ...
    lda_1100[0] = ARM_TO_DMA_BUS(&cb[37]);
    lda_1100[1] = ARM_TO_DMA_BUS(&cb[32]);
    lda_1c50[0] = ARM_TO_DMA_BUS(&cb[53]);
    lda_1c50[1] = ARM_TO_DMA_BUS(&cb[47]);
    lda_2c50[0] = ARM_TO_DMA_BUS(&cb[61]);
    lda_2c50[1] = ARM_TO_DMA_BUS(&cb[55]);
    lda_3c50[0] = ARM_TO_DMA_BUS(&cb[70]);
    lda_3c50[1] = ARM_TO_DMA_BUS(&cb[64]);
    ...
    cb[27].source_ad = ARM_TO_DMA_BUS(&row) + 0;
    cb[27].dest_ad = ARM_TO_DMA_BUS(&cb[28].source_ad) + 1;
    cb[27].txfr_len = 1;
    cb[27].nextconbk = ARM_TO_DMA_BUS(&cb[28]);

    // 1067: mult p10_offset_table.16 #4
    cb[28].ti = DMA_TI_WAIT_RESP;
    cb[28].source_ad = ARM_TO_DMA_BUS(lut_mult) + 4;
    cb[28].dest_ad = ARM_TO_DMA_BUS(&p10_offset_table) + 16;
    cb[28].txfr_len = 1;
    cb[28].nextconbk = ARM_TO_DMA_BUS(&cb[29]);

    // 1068: movw 1069 p10_offset_table.16
    cb[29].ti = DMA_TI_WAIT_RESP;
    cb[29].source_ad = ARM_TO_DMA_BUS(&p10_offset_table) + 16;
    cb[29].dest_ad = ARM_TO_DMA_BUS(&cb[30].source_ad) + 0;
    cb[29].txfr_len = 4;
    cb[29].nextconbk = ARM_TO_DMA_BUS(&cb[30]);

    // 1069: movw spi_dma_cb.4
    cb[30].ti = DMA_TI_WAIT_RESP;
    cb[30].source_ad = ARM_TO_DMA_BUS(lut_imm);
    cb[30].dest_ad = ARM_TO_DMA_BUS(&spi_dma_cb) + 4;
    cb[30].txfr_len = 4;
    ...
    cb[31].dest_ad = 0;
    cb[31].txfr_len = 0;
    cb[31].nextconbk = ARM_TO_DMA_BUS(&spi_dma_cb);
    // External DMA block at spi_dma_cb
    spi_dma_cb.nextconbk = ARM_TO_DMA_BUS(&cb[32]);
    // Wire in external DMA block

    // 1080: mov 1090.1 $0x7e204002
    cb[32].ti = DMA_TI_WAIT_RESP;
    cb[32].source_ad = 0x7e204002 + 0;
    cb[32].dest_ad = ARM_TO_DMA_BUS(&cb[33].source_ad) + 1;
    cb[32].txfr_len = 1;
    cb[32].nextconbk = ARM_TO_DMA_BUS(&cb[33]);

    // 1090: and 1100 #1
    cb[33].ti = DMA_TI_WAIT_RESP;
    cb[33].source_ad = ARM_TO_DMA_BUS(lut_and) + 1;
    cb[33].dest_ad = ARM_TO_DMA_BUS(&cb[34].source_ad) + 0;
    cb[33].txfr_len = 1;
    cb[33].nextconbk = ARM_TO_DMA_BUS(&cb[34]);

    // 1100: tez 1080
    cb[34].ti = DMA_TI_WAIT_RESP;
    cb[34].source_ad = ARM_TO_DMA_BUS(lut_tez);
    cb[34].dest_ad = ARM_TO_DMA_BUS(&cb[35].source_ad);
    cb[34].txfr_len = 1;
    cb[34].nextconbk = ARM_TO_DMA_BUS(&cb[35]);

    // 1100//lda: lda 1080
    cb[35].ti = DMA_TI_WAIT_RESP;
    cb[35].source_ad = ARM_TO_DMA_BUS(&lda_1100);
    cb[35].dest_ad = ARM_TO_DMA_BUS(&cb[36].nextconbk) + 0;
    cb[35].txfr_len = 4;
    ...
    cb[36].dest_ad = 0;
    cb[36].txfr_len = 0;
    cb[36].nextconbk = ARM_TO_DMA_BUS(&cb[32]);

    // 1110: movw word_temp $0x7e204000
    cb[37].ti = DMA_TI_WAIT_RESP;
    cb[37].source_ad = 0x7e204000 + 0;
    ...
    cb[38].source_ad = ARM_TO_DMA_BUS(&word_temp) + 0;
    cb[38].dest_ad = ARM_TO_DMA_BUS(&cb[39].source_ad) + 1;
    cb[38].txfr_len = 1;
    cb[38].nextconbk = ARM_TO_DMA_BUS(&cb[39]);

    // 1120: and word_temp #0b01111111
    cb[39].ti = DMA_TI_WAIT_RESP;
    cb[39].source_ad = ARM_TO_DMA_BUS(lut_and) + 0b01111111;
    cb[39].dest_ad = ARM_TO_DMA_BUS(&word_temp) + 0;
    cb[39].txfr_len = 1;
    cb[39].nextconbk = ARM_TO_DMA_BUS(&cb[40]);

    // 1130: movw $0x7e204000 word_temp
    cb[40].ti = DMA_TI_WAIT_RESP;
    cb[40].source_ad = ARM_TO_DMA_BUS(&word_temp) + 0;
    cb[40].dest_ad = 0x7e204000 + 0;
    cb[40].txfr_len = 4;
    cb[40].nextconbk = ARM_TO_DMA_BUS(&cb[41]);

    // 2000: mov word_temp.0 #0
    cb[41].ti = DMA_TI_WAIT_RESP;
    cb[41].source_ad = ARM_TO_DMA_BUS(lut_imm) + 0;
    cb[41].dest_ad = ARM_TO_DMA_BUS(&word_temp) + 0;
    cb[41].txfr_len = 1;
    cb[41].nextconbk = ARM_TO_DMA_BUS(&cb[42]);

    // 2001: mov word_temp.1 #0
    cb[42].ti = DMA_TI_WAIT_RESP;
    cb[42].source_ad = ARM_TO_DMA_BUS(lut_imm) + 0;
    cb[42].dest_ad = ARM_TO_DMA_BUS(&word_temp) + 1;
    cb[42].txfr_len = 1;
    cb[42].nextconbk = ARM_TO_DMA_BUS(&cb[43]);

    // 2002: mov word_temp.2 #0
    cb[43].ti = DMA_TI_WAIT_RESP;
    cb[43].source_ad = ARM_TO_DMA_BUS(lut_imm) + 0;
    cb[43].dest_ad = ARM_TO_DMA_BUS(&word_temp) + 2;
    cb[43].txfr_len = 1;
    cb[43].nextconbk = ARM_TO_DMA_BUS(&cb[44]);

    // 2003: mov word_temp.3 #1
    cb[44].ti = DMA_TI_WAIT_RESP;
    cb[44].source_ad = ARM_TO_DMA_BUS(lut_imm) + 1;
    cb[44].dest_ad = ARM_TO_DMA_BUS(&word_temp) + 3;
    cb[44].txfr_len = 1;
    cb[44].nextconbk = ARM_TO_DMA_BUS(&cb[45]);

    // 2004: movw $0x7e20001C word_temp
    cb[45].ti = DMA_TI_WAIT_RESP;
    cb[45].source_ad = ARM_TO_DMA_BUS(&word_temp) + 0;
    cb[45].dest_ad = 0x7e20001C + 0;
    cb[45].txfr_len = 4;
    cb[45].nextconbk = ARM_TO_DMA_BUS(&cb[46]);

    // 1c10: mov e $0x7e003004
    cb[46].ti = DMA_TI_WAIT_RESP;
    cb[46].source_ad = 0x7e003004 + 0;
    cb[46].dest_ad = ARM_TO_DMA_BUS(&e) + 0;
    cb[46].txfr_len = 1;
    cb[46].nextconbk = ARM_TO_DMA_BUS(&cb[47]);

    // 1c20: mov 1c40.1 e
    cb[47].ti = DMA_TI_WAIT_RESP;
    cb[47].source_ad = ARM_TO_DMA_BUS(&e) + 0;
    cb[47].dest_ad = ARM_TO_DMA_BUS(&cb[49].source_ad) + 1;
    cb[47].txfr_len = 1;
    cb[47].nextconbk = ARM_TO_DMA_BUS(&cb[48]);

    // 1c30: mov 1c40.0 $0x7e003004
    cb[48].ti = DMA_TI_WAIT_RESP;
    cb[48].source_ad = 0x7e003004 + 0;
    cb[48].dest_ad = ARM_TO_DMA_BUS(&cb[49].source_ad) + 0;
    cb[48].txfr_len = 1;
    cb[48].nextconbk = ARM_TO_DMA_BUS(&cb[49]);

    // 1c40: sub 1c50
    cb[49].ti = DMA_TI_WAIT_RESP;
    cb[49].source_ad = ARM_TO_DMA_BUS(lut_sub);
    cb[49].dest_ad = ARM_TO_DMA_BUS(&cb[50].source_ad) + 0;
    cb[49].txfr_len = 1;
    cb[49].nextconbk = ARM_TO_DMA_BUS(&cb[50]);

    // 1c50: tez 1c20
    cb[50].ti = DMA_TI_WAIT_RESP;
    cb[50].source_ad = ARM_TO_DMA_BUS(lut_tez);
    cb[50].dest_ad = ARM_TO_DMA_BUS(&cb[51].source_ad);
    cb[50].txfr_len = 1;
    cb[50].nextconbk = ARM_TO_DMA_BUS(&cb[51]);

    // 1c50//lda: lda 1c20
    cb[51].ti = DMA_TI_WAIT_RESP;
    cb[51].source_ad = ARM_TO_DMA_BUS(&lda_1c50);
    cb[51].dest_ad = ARM_TO_DMA_BUS(&cb[52].nextconbk) + 0;
    cb[51].txfr_len = 4;
    ...
    cb[52].dest_ad = 0;
    cb[52].txfr_len = 0;
    cb[52].nextconbk = ARM_TO_DMA_BUS(&cb[47]);

    // 2010: movw $0x7e200028 word_temp
    cb[53].ti = DMA_TI_WAIT_RESP;
    cb[53].source_ad = ARM_TO_DMA_BUS(&word_temp) + 0;
    cb[53].dest_ad = 0x7e200028 + 0;
    cb[53].txfr_len = 4;
    cb[53].nextconbk = ARM_TO_DMA_BUS(&cb[54]);

    // 2c10: mov e $0x7e003004
    cb[54].ti = DMA_TI_WAIT_RESP;
    cb[54].source_ad = 0x7e003004 + 0;
    cb[54].dest_ad = ARM_TO_DMA_BUS(&e) + 0;
    cb[54].txfr_len = 1;
    cb[54].nextconbk = ARM_TO_DMA_BUS(&cb[55]);

    // 2c20: mov 2c40.1 e
    cb[55].ti = DMA_TI_WAIT_RESP;
    cb[55].source_ad = ARM_TO_DMA_BUS(&e) + 0;
    cb[55].dest_ad = ARM_TO_DMA_BUS(&cb[57].source_ad) + 1;
    cb[55].txfr_len = 1;
    cb[55].nextconbk = ARM_TO_DMA_BUS(&cb[56]);

    // 2c30: mov 2c40.0 $0x7e003004
    cb[56].ti = DMA_TI_WAIT_RESP;
    cb[56].source_ad = 0x7e003004 + 0;
    cb[56].dest_ad = ARM_TO_DMA_BUS(&cb[57].source_ad) + 0;
    cb[56].txfr_len = 1;
    cb[56].nextconbk = ARM_TO_DMA_BUS(&cb[57]);

    // 2c40: sub 2c50
    cb[57].ti = DMA_TI_WAIT_RESP;
    cb[57].source_ad = ARM_TO_DMA_BUS(lut_sub);
    cb[57].dest_ad = ARM_TO_DMA_BUS(&cb[58].source_ad) + 0;
    cb[57].txfr_len = 1;
    cb[57].nextconbk = ARM_TO_DMA_BUS(&cb[58]);

    // 2c50: tez 2c20
    cb[58].ti = DMA_TI_WAIT_RESP;
    cb[58].source_ad = ARM_TO_DMA_BUS(lut_tez);
    cb[58].dest_ad = ARM_TO_DMA_BUS(&cb[59].source_ad);
    cb[58].txfr_len = 1;
    cb[58].nextconbk = ARM_TO_DMA_BUS(&cb[59]);

    // 2c50//lda: lda 2c20
    cb[59].ti = DMA_TI_WAIT_RESP;
    cb[59].source_ad = ARM_TO_DMA_BUS(&lda_2c50);
    cb[59].dest_ad = ARM_TO_DMA_BUS(&cb[60].nextconbk) + 0;
    cb[59].txfr_len = 4;
    ...
    cb[60].dest_ad = 0;
    cb[60].txfr_len = 0;
    cb[60].nextconbk = ARM_TO_DMA_BUS(&cb[55]);

    // 2020: mov word_temp.3 #2
    cb[61].ti = DMA_TI_WAIT_RESP;
    cb[61].source_ad = ARM_TO_DMA_BUS(lut_imm) + 2;
    cb[61].dest_ad = ARM_TO_DMA_BUS(&word_temp) + 3;
    cb[61].txfr_len = 1;
    cb[61].nextconbk = ARM_TO_DMA_BUS(&cb[62]);

    // 2021: movw $0x7e20001C word_temp
    cb[62].ti = DMA_TI_WAIT_RESP;
    cb[62].source_ad = ARM_TO_DMA_BUS(&word_temp) + 0;
    cb[62].dest_ad = 0x7e20001C + 0;
    cb[62].txfr_len = 4;
    cb[62].nextconbk = ARM_TO_DMA_BUS(&cb[63]);

    // 3c10: mov e $0x7e003005
    cb[63].ti = DMA_TI_WAIT_RESP;
    cb[63].source_ad = 0x7e003005 + 0;
    cb[63].dest_ad = ARM_TO_DMA_BUS(&e) + 0;
    cb[63].txfr_len = 1;
    cb[63].nextconbk = ARM_TO_DMA_BUS(&cb[64]);

    // 3c20: mov 3c40.1 e
    cb[64].ti = DMA_TI_WAIT_RESP;
    cb[64].source_ad = ARM_TO_DMA_BUS(&e) + 0;
    cb[64].dest_ad = ARM_TO_DMA_BUS(&cb[66].source_ad) + 1;
    cb[64].txfr_len = 1;
    cb[64].nextconbk = ARM_TO_DMA_BUS(&cb[65]);

    // 3c30: mov 3c40.0 $0x7e003005
    cb[65].ti = DMA_TI_WAIT_RESP;
    cb[65].source_ad = 0x7e003005 + 0;
    cb[65].dest_ad = ARM_TO_DMA_BUS(&cb[66].source_ad) + 0;
    cb[65].txfr_len = 1;
    cb[65].nextconbk = ARM_TO_DMA_BUS(&cb[66]);

    // 3c40: sub 3c50
    cb[66].ti = DMA_TI_WAIT_RESP;
    cb[66].source_ad = ARM_TO_DMA_BUS(lut_sub);
    cb[66].dest_ad = ARM_TO_DMA_BUS(&cb[67].source_ad) + 0;
    cb[66].txfr_len = 1;
    cb[66].nextconbk = ARM_TO_DMA_BUS(&cb[67]);

    // 3c50: tez 3c20
    cb[67].ti = DMA_TI_WAIT_RESP;
    cb[67].source_ad = ARM_TO_DMA_BUS(lut_tez);
    cb[67].dest_ad = ARM_TO_DMA_BUS(&cb[68].source_ad);
    cb[67].txfr_len = 1;
    cb[67].nextconbk = ARM_TO_DMA_BUS(&cb[68]);

    // 3c50//lda: lda 3c20
    cb[68].ti = DMA_TI_WAIT_RESP;
    cb[68].source_ad = ARM_TO_DMA_BUS(&lda_3c50);
    cb[68].dest_ad = ARM_TO_DMA_BUS(&cb[69].nextconbk) + 0;
    cb[68].txfr_len = 4;
    ...
    cb[69].dest_ad = 0;
    cb[69].txfr_len = 0;
    cb[69].nextconbk = ARM_TO_DMA_BUS(&cb[64]);

    // 2030: movw $0x7e200028 word_temp
    cb[70].ti = DMA_TI_WAIT_RESP;
    cb[70].source_ad = ARM_TO_DMA_BUS(&word_temp) + 0;
    cb[70].dest_ad = 0x7e200028 + 0;
    cb[70].txfr_len = 4;
    cb[70].nextconbk = ARM_TO_DMA_BUS(&cb[71]);

    // 3000: mov 3010.1 row
    cb[71].ti = DMA_TI_WAIT_RESP;
    cb[71].source_ad = ARM_TO_DMA_BUS(&row) + 0;
    cb[71].dest_ad = ARM_TO_DMA_BUS(&cb[72].source_ad) + 1;
    cb[71].txfr_len = 1;
    cb[71].nextconbk = ARM_TO_DMA_BUS(&cb[72]);

    // 3010: add 3020.1 #1
    cb[72].ti = DMA_TI_WAIT_RESP;
    cb[72].source_ad = ARM_TO_DMA_BUS(lut_add) + 1;
    cb[72].dest_ad = ARM_TO_DMA_BUS(&cb[73].source_ad) + 1;
    cb[72].txfr_len = 1;
    cb[72].nextconbk = ARM_TO_DMA_BUS(&cb[73]);

    // 3020: and 3030 #3
    cb[73].ti = DMA_TI_WAIT_RESP;
    cb[73].source_ad = ARM_TO_DMA_BUS(lut_and) + 3;
    cb[73].dest_ad = ARM_TO_DMA_BUS(&cb[74].source_ad) + 0;
    cb[73].txfr_len = 1;
    cb[73].nextconbk = ARM_TO_DMA_BUS(&cb[74]);
    ...
    cb[74].txfr_len = 1;
    cb[74].nextconbk = ARM_TO_DMA_BUS(&cb[75]);

    // 4000: j 0000 @ loop
    cb[75].ti = DMA_TI_WAIT_RESP;
    cb[75].source_ad = 0;
    cb[75].dest_ad = 0;
    cb[75].txfr_len = 0;
    cb[75].nextconbk = ARM_TO_DMA_BUS(&cb[0]);

    // 114514: jdma p10_offset_table
    cb[76].ti = DMA_TI_WAIT_RESP;
    cb[76].source_ad = 0;
    cb[76].dest_ad = 0;
    cb[76].txfr_len = 0;
    cb[76].nextconbk = ARM_TO_DMA_BUS(&p10_offset_table);
    // External DMA block at p10_offset_table
    p10_offset_table.nextconbk = ARM_TO_DMA_BUS(&cb[77]);
    // Wire in external DMA block

    // 114515: jdma spi_dma_cb_rx_addr
    cb[77].ti = DMA_TI_WAIT_RESP;
    cb[77].source_ad = 0;
    cb[77].dest_ad = 0;
    cb[77].txfr_len = 0;
    cb[77].nextconbk = ARM_TO_DMA_BUS(&spi_dma_cb_rx_addr);
    // External DMA block at spi_dma_cb_rx_addr
    spi_dma_cb_rx_addr.nextconbk = ARM_TO_DMA_BUS(&cb[78]);
    // Wire in external DMA block

    // 13030: mov row
    cb[78].ti = DMA_TI_WAIT_RESP;
    cb[78].source_ad = ARM_TO_DMA_BUS(lut_imm);
    cb[78].dest_ad = ARM_TO_DMA_BUS(&row) + 0;
    cb[78].txfr_len = 1;
    cb[78].nextconbk = 0;
